...
public class GameProfileBuilder {
    ...
    public static GameProfile fetch(UUID uuid, boolean forceNew) throws IOException {
        ...
        throw new IOException(error
                .get("error").getAsString() + ": " + error.get("errorMessage").getAsString());
    }
    ...
    public static GameProfile getProfile(UUID uuid, String name, String skinUrl, String capeUrl) {
        ...
        profile.getProperties().put("textures", new Property("textures",
                Base64Coder.encodeString(String.format(cape ? "{\"timestamp\":%d,\"profileId\":\"%s\",\"profileName\":\"%s\",\"isPublic\":true,\"textures\":{\"SKIN\":{\"url\":\"%s\"},\"CAPE\":{\"url\":\"%s\"}}}" : "{\"timestamp\":%d,\"profileId\":\"%s\",\"profileName\":\"%s\",\"isPublic\":true,\"textures\":{\"SKIN\":{\"url\":\"%s\"}}}", args.toArray(new Object[args.size()])))));
        return profile;
    }
    ...
    private static class GameProfileSerializer implements JsonSerializer<GameProfile>, JsonDeserializer<GameProfile> {
        ...
        public GameProfile deserialize(JsonElement json, Type type, JsonDeserializationContext context) throws JsonParseException {
            ...
            if (object.has("properties"))
                for (Map.Entry<String, Property> prop : (Iterable<Map.Entry<String, Property>>)((PropertyMap)context.deserialize(object.get("properties"), PropertyMap.class))
                        .entries())
    ...
    private static class CachedProfile {
        ...
        public boolean isValid() {
            return (GameProfileBuilder.cacheTime < 0L) ? true : ((System.currentTimeMillis() - this.timestamp < GameProfileBuilder.cacheTime));
        }
    }
}
